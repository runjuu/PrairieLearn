...
| Rendering           | `document.test.ts`: shared document policy and both Source Question Type adapters; `server.test.ts`: deterministic default seed, both render modes and current-file refresh; `standalonePreview.spec.ts`: Chromium hydration for all six Source Question Types, deterministic seeds and source refresh                                                                                                     |
| Grading             | `document.test.ts`: Freeform and legacy submissions, authoritative legacy seed regeneration, Internal/External/Manual boundaries and stateless grading; `server.test.ts`: no retained submission state; `standalonePreview.spec.ts`: both native browser submission contracts                                                                                                                              |
