...
(() => {
  ...
  const stateBadge = document.getElementById('workspace-state');
  ...
  function applyStatus(status) {
    ...
    stateBadge.className = 'badge ' + (status.state === 'running'
      ? 'text-bg-success'
      : status.state === 'failed'
        ? 'text-bg-danger'
        : 'text-bg-secondary');
      ...
      <body class="d-flex flex-column h-100">
        ...
        >
          <nav class="navbar navbar-expand navbar-dark bg-dark px-3">
            <span class="navbar-brand">${entry.spec.qid}</span>
            <span
              id="workspace-state"
              class="badge ${running ? 'text-bg-success' : 'text-bg-secondary'}"
            >
              ${entry.state}
            </span>
            <span id="workspace-message" class="navbar-text ms-2 me-auto">${entry.message}</span>
            <a class="btn btn-outline-light btn-sm me-2" href="${urls.questionUrl}">
              Back to question
            </a>
            <form class="me-2" method="POST" action="${urls.actionUrl}">
              <button
                type="submit"
